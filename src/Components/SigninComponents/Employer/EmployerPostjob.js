...
function Postjob() {
  ...
  useEffect(() => {
    document.title = formm.title ? `${formm.title} Job in ${formm.location} - Apply Now!` : 'Post a Job';
  }, [formm.title, formm.location]);
  ...
  async function clicked() {
    if (formm.title.length > 4 || formm.location.length >= 2 || formm.rate || formm.min > 0 || formm.max > 0 || formm.description.length > 4 || formm.time) {
      ...
      if (check) {
        ...
        await setDoc(doc(db, 'employer', check.id), { jobpostings: arrayUnion(formm) }, { merge: true });
  ...
  return (
    <div>
      <Helmet>
        <title>{formm.title ? `${formm.title} Job in ${formm.location} - Apply Now!` : 'Post a Job'}</title>
        <meta name="description" content={formm.description ? formm.description : 'Post a job on our platform and find the right candidate quickly and efficiently.'} />
        <meta name="keywords" content={`${formm.title}, ${formm.location}, job posting, job search, employment`} />
      </Helmet>
      <>
        <EmployerNav />
        {selector &&
          <div className='largedivpostjob'>
            ...
            <div className='postjobsubdiv2'>
              ...
              <TextareaAutosize
                ...
                variant="outlined"
                style={{ minHeight: '50px', maxWidth: '100%', minWidth: '100%', border: '1px solid rgb(22, 64, 129)', outline: 'none', fontSize: '18px', fontFamily: 'roboto', borderRadius: '10px' }}
